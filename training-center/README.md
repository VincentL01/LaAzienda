...
4. Let Aurelia observe the imported whole-tree digest, then unlock the Training Room with `runtime/Copy-CeoTrainingCredential.ps1` and explicitly approve that exact revision. The script copies the independent ignored `assets/owner/runtime/credential`, never the runtime bridge token. Docker stores only its one-way verifier. Successful login creates a random eight-hour HttpOnly, same-site session token; D1 stores only the token hash and expiry, while the credential itself is never stored in browser JavaScript, D1, a URL, a log, or Docker configuration.
